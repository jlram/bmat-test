...
class WorkViewSetTest(TestCase):
    ...
    def test_delete_work(self):
        ...
        response = client.delete('/works/' + str(instance.iswc) + '/')
...
class SourceViewSetTest(TestCase):
    ...
    def test_update_source(self):
        print('test_update_source')
        instance = Source.objects.filter(name='Sony').first()
        work = Work.objects.first()
        response = client.put('/sources/' + str(instance.id) + '/', {'work': work.id}, content_type='application/json')
        self.assertEqual(response.data['work'], work.id)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
